...
(function(global, factory) {
    ...
    function brush$1(dim) {
        ...
        brush.extent = function(_) {
            return arguments.length ? (extent = typeof _ === "function" ? _ : constant$3([
                [+_[0][0], +_[0][1]],
                [+_[1][0], +_[1][1]]
            ]), brush) : extent;
        };
    ...
    var computeRealCentroids = function computeRealCentroids(par, position) {
        return function(row) {
            return Object.keys(par.dimensions).map(function(d) {
                var x = position(d);
                var y = par.dimensions[d].yscale(row[d]);
                return [x, y];
            });
        };
    };
    ...
    var createAxes = function createAxes(config, pc, xscale, flags, axis) {
        return function() {
            ...
            pc._g.append('svg:g').attr('class', d => {
                return 'axis axis-' + d;
            }).attr('transform', 'translate(0,0)').each(function(d) {
                var axisElement = select(this).call(pc.applyAxisConfig(axis, config.dimensions[d]));

                axisElement.selectAll('path').attr("class", "path-" + d).style('fill', 'none').style('stroke', '#222').style('shape-rendering', 'crispEdges');

                axisElement.selectAll('line').attr("class", "line-" + d).style('fill', 'none').style('stroke', '#222').style('shape-rendering', 'crispEdges');
    ...
    var reorderable = function reorderable(config, pc, xscale, position, dragging, flags, events) { // TODO: update with events to detect axes reorder
        return function() {
    ...
    var highlight = function highlight(config, pc, canvas, events, ctx, position) {
        return function() {
            ...
            pc.clear('highlight');
            selectAll([canvas.foreground]).classed('faded', true);
    ...
    // highlight an array of data
    var highlight2 = function highlight2(config, pc, canvas, events, ctx, position) {
        return function() {
            var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
    ...
    var bresenham = function(x0, y0, x1, y1, fn) {
        if (!fn) {
            var arr = [];
            fn = function fn(x, y) {
                arr.push({ x: x, y: y });
    ...
    var singlePathTile = function singlePathTile(config, pc, position) {
        return function(d) {
            ...
            points.forEach(function(p, i) {
                ...
                var scalegx = quantize$1();
                var scalegy = quantize$1();
    ...
    var ParCoords = function ParCoords(userConfig) {
        ...
        pc.reorderable = reorderable(config, pc, xscale, position, dragging, flags, events); // TODO add events to detect end of reorder
        ...
        // highlight an array of data
        pc.highlight2 = highlight2(config, pc, canvas, events, ctx, position);
        // clear highlighting
