...
Vue.component('gc-map', {
    ...
    watch: {
        ...
        currentRasterIndex: function(newValue, oldValue) {
            ...
            if (newValue != oldValue) {
                ...
                if (this.getCurrentRaster()) {
                    this.getTimeForChart();
    ...
    methods: {
        ...
        createParcelAction: function() {
            ...
            } catch (err) { console.debug("could not disable query button."); }
        ...
        initTimeline: function() {
            ...
            var _visOptions = {
                ...
                snap: function(date, scale, step) {
                    // console.log("----");
                    // TODO-R
                    // console.log(Math.round(date / MS_IN_A_DAY) * MS_IN_A_DAY)
            ...
            this.timeLine.on("timechanged", function(a) {
                ...
                // TODO-R
                this.getTimeForChart(a.time);
                // find nearest date to given date
                let days = parcel.timeseries.map(r => new Date(r.date));
                let testDate = a.time;
                let closestDate = days.length;
        ...
        getTimeForChart(date) {
            if (date) {
                setGlobalDate(date);
            } else {
                setGlobalDate(this.getCurrentParcel().timeseries[this.currentTimeSliderPosition].date);
            }
        }
    }
});